...
import { LayoutDashboard, Package, Plus, LogOut, Menu, X, Box } from 'lucide-react';
...
export default function App() {
  ...
  const NavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
    ...
    >
      <Icon className={`w-5 h-5 mr-3 ${currentView === view ? 'text-indigo-600' : 'text-gray-400'}`} />
      {label}
    </button>
  );
      ...
      <main className="flex-1 overflow-y-auto h-screen pt-16 md:pt-0">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
            ...
            {(currentView === 'inventory' || currentView === 'dashboard') && (
              <button
